...
if(isset($_GET['id']) AND !empty($_GET['id'])){ // On vérifie que l'id de l'utilisateur est dans l'url
    ...
    if($recupUser->rowCount()>0){ // S'il il correspond, on le passe à l'état de membre de la base de donnée
        $updateUser = $bdd->prepare('UPDATE users SET admin=0 WHERE id = ?');
