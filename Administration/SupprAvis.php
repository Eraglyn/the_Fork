<?php
session_start();
$bddA = new PDO('mysql:host=localhost;dbname=espace_restaurant;charset=utf8;', 'root', ''); // Connexion à la base de donnée des avis
...
if(isset($_GET['id']) AND !empty($_GET['id'])){ // On vérifie que l'id de l'avis est dans l'url
    $id=$_GET['id'];
    $recupUser= $bddA->prepare('SELECT * FROM avis_restaurant WHERE id = ?'); // On vérifie qu'il correspond à un avis dans la base de donnée
    $recupUser->execute(array($id));
    if($recupUser->rowCount()>0){ // S'il il correspond, on le supprime de la base de donnée
        $bannirUser=$bddA->prepare('DELETE FROM avis_restaurant WHERE id=?');
        ...
        echo "Cet identifiant n'appartiens à aucun avis.";
    }
}else{
